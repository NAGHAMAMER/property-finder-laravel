...
<script>
    (() => {
        const loginToken = <?php echo json_encode($loginToken, 15, 512) ?>;
        const userData = <?php echo json_encode($userData, 15, 512) ?>;

        if (!loginToken || !userData) {
            window.location.replace(<?php echo json_encode(route('user.login'), 15, 512) ?>);
            return;
        }

        localStorage.setItem('auth_token', loginToken);
        localStorage.setItem('auth_user', JSON.stringify(userData));
        window.location.replace(<?php echo json_encode(route('user.dashboard'), 15, 512) ?>);
    })();
</script>
</body>
</html>
<?php /**PATH C:\Users\نغم\Downloads\property-finder-user-web-api-final\property-finder-user-web-api-final\property_finder_final\resources\views/user/auth/password-reset-complete.blade.php ENDPATH**/ ?>